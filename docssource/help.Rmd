...
Factors are handled by decomposing the variable in K-1 contrast variable and insert in the model in place of the categorical variable. The type of contrast used, for each factor, can be seen in the `r ("Factors Coding")` tab.
...
A model with two endogenous variables, one continuous and one factor as exogenous predictors is sets as follows:
...
`r opt("Estimation")` regards the estimator. At the moment, there are the following estimators available, selected by `r opt("Method")` combo-box:
...
The options `r opt("R-squared tests")` provides Chi-squared tests for the $R2$. Details can be found here [Computation details](compute_details.html)
