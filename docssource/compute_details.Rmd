...
Standard errors are slightly different, but that is due to the fact that regression uses OLS and path analysis uses ML. We also get the same $R^2$ and comparable p-value for the $R^2$, which is discussed [below](#(R^2)) .



# $R^2$ 

$R^2$ are computed as one minus the standardized residual variance of each endogenous variable. Pratically, defining $\hat{\sigma}^2_r$ as the standardized residual variance  $R^2_v=1-\hat{\sigma}^2_r$ 

As regards the confidence intervals, by default they are estimated converting $R^2$ to $R$, then to $z$ using the Fisher Z-transformation, then estimating the C.I. for $z$ and back transform them to $R^2$. Details in @r2ci.
